...
```text
entrega_preprocessamento_ciencia_dados/
│
├── app.py
...
```bash
python preprocessamento_credito.py
```

Caso os arquivos CSV estejam em outra pasta, use o parâmetro `--base-dir`:

```bash
python preprocessamento_credito.py --base-dir "CAMINHO_DA_PASTA"
```

Exemplo no Windows:

```powershell
python preprocessamento_credito.py --base-dir "C:\Users\SeuUsuario\Desktop\entrega_preprocessamento_ciencia_dados"
```
